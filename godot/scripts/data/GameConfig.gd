...
# ── Upgrade XP thresholds (Ou=[812,2275,3737] from original + 4th estimate) ──
const UPGRADE_XP := [812, 2275, 3737, 5500]
# Corresponding level gates (used only for display / UI labels)
const UPGRADE_LEVELS := [5, 15, 30, 45]

# ── Level system (XP for each cumulative level, derived from original) ────────
# Pattern: Ou[0]=812→level5, Ou[1]=2275→level15, Ou[2]=3737→level30, ~5500→level45
const XP_TABLE: Array = [
	0, 40, 100, 175, 280, 430,       # levels 0-5
	610, 820, 1070, 1360, 1700,       # levels 6-10
	2090, 2540, 2980, 3200, 3600,     # levels 11-15 (upgrade at 15≈2275 cumul → use table)
...
# ── Upgrade tier colors ────────────────────────────────────────────────────────
const TIER_COLORS := {
	0: Color("#e0e0e0"),   # T0 Basic — grey
	1: Color("#22cc55"),   # T1 Level 5  — green
	2: Color("#ffdd00"),   # T2 Level 15 — yellow
	3: Color("#ff8800"),   # T3 Level 30 — orange
	4: Color("#ff2200")    # T4 Level 45 — red
}
