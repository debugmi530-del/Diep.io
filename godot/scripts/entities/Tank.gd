...
func apply_tank_class(class_name_: String) -> void:
        ...
        var hp_mult: float = tank_data.get("hp_mult", tank_data.get("hp_multiplier", 1.0))
        var spd_mult: float = tank_data.get("speed_mult", tank_data.get("speed_multiplier", 1.0))
        var r_mult: float = tank_data.get("radius_mult", tank_data.get("radius_multiplier", 1.0))
        ...
        body_damage = GameConfig.BASE_BODY_DAMAGE * tank_data.get("body_dmg_mult", tank_data.get("body_damage_multiplier", 1.0))
...
func get_barrel_world_pos(barrel_idx: int) -> Vector2:
        ...
        var angle: float = aim_angle + b.get("ao", b.get("angle_offset", 0.0))
        if auto_spin:
                angle += spin_angle
        var lat: float = b.get("lat", b.get("lateral_offset", 0.0))
        var blen: float = b.get("len", b.get("length", 48.0))
        var dir = Vector2(cos(angle), sin(angle))
        var perp = Vector2(-sin(angle), cos(angle))
        return global_position + dir * blen + perp * lat
...
func get_barrel_angle(barrel_idx: int) -> float:
        ...
        if barrel_idx >= barrels.size():
                return aim_angle
        var b: Dictionary = barrels[barrel_idx]
        var base_angle = b.get("ao", b.get("angle_offset", 0.0))
        if auto_spin:
...
func _draw_tank() -> void:
        ...
        for i in barrels.size():
                ...
                var angle: float = b.get("ao", b.get("angle_offset", 0.0))
                if auto_spin:
                        angle += spin_angle
                var full_angle = aim_angle + angle
                var blen: float = b.get("len", b.get("length", 48.0))
                var bwid: float = b.get("wd", b.get("width", 14.0))
                var lat: float = b.get("lat", b.get("lateral_offset", 0.0))
