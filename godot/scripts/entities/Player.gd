...
func _fire_barrel(barrel_idx: int) -> void:
        ...
        var spread: float = b.get("sp", b.get("spread", 0.0))
        if spread > 0.0:
                angle += randf_range(-spread, spread)
        var bullet_type: String = "normal"
        if b.get("is_sticky", false): bullet_type = "sticky"
        elif b.get("is_bomb", false): bullet_type = "bomb"
        elif b.get("is_trap", false): bullet_type = "trap"
        var bdm: float = b.get("bdm", b.get("bullet_damage_multiplier", 1.0))
        var bsp: float = b.get("bsp", b.get("bullet_speed_multiplier", 1.0))
        var bsm: float = b.get("bsm", b.get("bullet_size_multiplier", 1.0))
        game.spawn_bullet(
                get_barrel_world_pos(barrel_idx),
                angle,
                GameConfig.BASE_BULLET_DAMAGE * bdm,
                GameConfig.BASE_BULLET_SPEED * bsp,
                GameConfig.BASE_RADIUS * bsm * 0.5,
                self,
                bullet_type
        )
        # Recoil
...
func _on_level_up() -> void:
        super._on_level_up()
        # Check if this level corresponds to an upgrade gate (L5, L15, L30, L45)
        var is_upgrade_level := false
        for ul in GameConfig.UPGRADE_LEVELS:
                if level == ul:
                        is_upgrade_level = true
                        break
        if is_upgrade_level:
                var available = TankData.get_upgrades_for(tank_class, xp)
                if available.size() > 0:
                        is_upgrading = true
